  ...
  setup(props) {
    ...
    const onDeleteTweet = async () => {
      if (confirm('정말로 트윗을 삭제하시겠습니까?')) {
        ...
        // delete likes
        const likeSnapshot = await LIKES_COLLECTION.where('from_tweet_id', '==', props.tweet.id).get()
        likeSnapshot.docs.forEach(async (doc) => await doc.ref.delete())
        // delete retweets
        const retweetCollection = await RETWEET_COLLECTION.where('from_tweet_id', '==', props.tweet.id).get()
        retweetCollection.docs.forEach(async (doc) => await doc.ref.delete())
        // user collection - num_tweets (-1)
        await USER_COLLECTION.doc(props.tweet.uid).update({
