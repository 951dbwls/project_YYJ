  ...
  <div class="flex-1 flex">
    <div class ="flex-1 border-r border-gray-100">
      <div class="flex flex-col" v-if="tweet">
        <!--title-->
        <div class="flex items-center px-3 py-2 border-gray-100">
        ...
        <div class="flex justify-around py-2">
          ...
          <button @click="handleRetweet(tweet)">
            <i v-if="tweet.isRetweeted" class="fas fa-retweet text-xl hover:bg-green-50 text-green-300 p-2 rounded-full h-10 w-10"></i>
            <i v-else class="fas fa-retweet text-gray-400 text-xl hover:bg-green-50 hover:text-green-300 p-2 rounded-full h-10 w-10"></i>
          </button>
          <button @click="handleLikes(tweet)">
            <i v-if="tweet.isLiked" class="far fa-heart text-xl hover:bg-red-50 text-red-300 p-2 rounded-full h-10 w-10"></i>
            <i v-else class="far fa-heart text-gray-400 text-xl hover:bg-red-50 hover:text-red-300 p-2 rounded-full h-10 w-10"></i>
          </button>
        </div>
        <div class="h-px w-full bg-gray-100"></div>
        <!-- comments -->
...
import hadleRetweet from '../utils/handleRetweet'
...
export default {
  ...
  setup(){
    ...
    onBeforeMount(async () => {
      ...
      COMMENT_COLLECTION.where('from_tweet_id', '==', route.params.id).orderBy('created_at', 'desc').onSnapshot(snapshot => {
    ...
    return{router, tweet, comments, currentUser, moment, showCommentModal, hadleRetweet, handleLikes, handleDeleteComment}
  },
}
</script>
